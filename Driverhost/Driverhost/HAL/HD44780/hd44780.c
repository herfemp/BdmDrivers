...
void Delay_us(uint32_t del){
	//uint32_t tmr; uint32_t us;
	///< 1~ us
	///< Can't be bothered with DWT timer for now
	LCDTMR=(del/1000)+1;
	while (LCDTMR);
	...
	/*for(us=0; us<del; us++){
		...
		for(tmr=0; tmr< (F_CPU/10); tmr++){
			;
		}
	}*/
}
///< Really? Talk about unnecessary..
void Delay_ns(uint32_t del){ Delay_us(del); }
void Delay_ms(uint32_t del){

	LCDTMR=del+1;
	while (LCDTMR);
	//for(tmr=0; tmr<del; tmr++){ Delay_us(1000); } ///< 1~ ms
}
#endif
#endif
...
Returns:  none
///< Modified this a tad so one can chose how many characters to print; Len = 0 will bypass that function /Chriva
*************************************************************************/
void lcd_puts(const char *s, uint8_t Len){
...
Returns:  none
*************************************************************************/
/*void lcd_puts_P(const char *progmem_s, uint8_t Len)
  {
    register char c;
	if(Len==0){
...
void lcd_puts_E(const uint8_t *s, uint8_t Len){
