...
#define CS_INIT()	SetPinDir(SDCARD_CS_PORT, SDCARD_CS_PIN, 1)//DDRD  |= 0b00000100	/* Initialize port for MMC CS as output */
// #define	CS_H()		WritePin (SDCARD_CS_PORT, SDCARD_CS_PIN, 1)//PORTD |= (1 << PORTD2)
...
void dly_us (UINT n){
	//uint32_t delay;
	for(n=0; n<F_CPU; n++){
		; ///< *cough* *cough* *Ahem* Not a nice way :D
	}
}*/

void dly_us(uint32_t del){
	uint32_t tmr;
	uint32_t us;
	for(us=0; us<del; us++){
		///< 1~ us
		for(tmr=0; tmr<48; tmr++){ ; }
	}
}
